(function(){
 ...
 window.showEtiquetteReviewHistory=function(){
  ...
  var renderDialogue=function(dialogue,feedback){
   if(!dialogue||typeof dialogue!=='object')return '';
   var keys=Object.keys(dialogue).sort(function(a,b){return Number(a)-Number(b);});
   if(!keys.length)return '';
   return '<details style="margin-top:9px;border-top:1px solid #eef2f7;padding-top:8px;"><summary style="cursor:pointer;color:#4f46e5;font-size:12px;font-weight:600;">查看逐轮对话与教练提示</summary><div style="margin-top:8px;">'+keys.map(function(k,i){var tip=feedback&&feedback[k];return '<div style="margin:7px 0;padding:8px 10px;border-radius:9px;background:#f8fafc;"><small style="color:#94a3b8;">第'+(i+1)+'轮</small><div style="color:#334155;font-size:12px;line-height:1.55;margin-top:3px;">'+esc(dialogue[k])+'</div>'+(tip?'<div style="margin-top:5px;color:#b45309;font-size:11px;line-height:1.5;">'+esc(tip.title)+'：'+esc(tip.desc)+'</div>':'')+'</div>';}).join('')+'</div></details>';
  };
  var rows=logs.slice(0,8).map(function(x){
   ...
   return '<article style="padding:12px;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:8px;background:#fff;"><b style="color:#4338ca;">'+esc(x.scene||'礼仪训练')+'</b><span style="display:block;font-size:11px;color:#64748b;margin-top:4px;">'+esc([x.location,x.outcome,x.feeling].filter(Boolean).join(' · ')||'未填写地点/结果')+' · '+(x.createdAt?new Date(x.createdAt).toLocaleDateString():'')+'</span><p style="font-size:12px;color:#374151;line-height:1.6;margin:6px 0;">'+esc(x.note||'暂无文字记录')+'</p><div style="margin:7px 0;padding:8px 10px;border-radius:9px;background:#ecfdf5;color:#065f46;font-size:11px;line-height:1.55;"><b>现实挑战：</b>'+esc(x.challenge||'把本课动作带到现实场景')+(x.branch?'<br><b>本次分支：</b>'+esc(x.branch):'')+'</div><small style="color:#64748b;">AI 对话：'+turns+' 轮 · 行为完成：'+behavior+'%</small>'+renderDialogue(x.dialogue,x.dialogueFeedback)+'<small style="display:block;color:#0369a1;margin-top:6px;">知识自查：'+(x.courseKnowledgeScore==null?'未进行':String(x.courseKnowledgeScore)+'/100')+'</small><small style="display:block;color:#b45309;margin-top:6px;">复盘提示：'+esc(tip)+'</small><small style="display:block;color:#047857;margin-top:4px;">下一步：'+esc(x.recommendation||'继续保持练习')+'</small></article>';
  }).join('');
