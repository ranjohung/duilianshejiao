(function(){
 function readRecommendation(){
  ...
  if(rec.recommendation)return {title:rec.recommendation,reason:'来自上一节课的现实记录，继续练习最薄弱的行为环节。',source:rec.scene||'礼仪训练'};
  try{rec=JSON.parse(localStorage.getItem('duilian_etiquette_next_recommendation')||'{}')}catch(e){}
  return {title:rec.title||'低压力问候练习',reason:rec.reason||'先在熟悉的人身上完成一次自然问候，再逐步增加场景复杂度。',source:rec.scene||'礼仪训练'};
 }
 function enhance(){
