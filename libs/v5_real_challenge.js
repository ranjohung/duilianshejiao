...
(function () {
  ...
  const SCENE_RULES = {
    ...
    // ===== 新增: 来自 PPT 的商务场景规则 =====
    '商务会面九部曲': {
      mustHave: ['微笑', '眼神接触', '握手', '称呼', '自我介绍'],
      niceToHave: ['双手接递名片', '引领', '位次安排'],
      forbidden: ['无视对方', '不握手', '不回应', '玩手机'],
      tips: '商务接待核心是"九部曲顺序+每个环节动作标准"：迎候→握手（7步骤）→介绍→名片→称呼→引领→位次→茶水→送别。礼者，敬人也；仪者，规则标准也。'
    },
    '商务宴请全流程': {
      mustHave: ['座次安排', '饮食禁忌确认', '敬酒分寸'],
      niceToHave: ['提前结账', '话题选择'],
      forbidden: ['劝酒', '座次混乱', '铺张浪费'],
      tips: '商务宴请核心是"尊重而非劝酒"：面门为上、右为尊；饮食提前确认禁忌；饮酒按对方意愿；结账主方提前处理。'
    },
    '政务礼仪规范': {
      mustHave: ['准确称呼', '结构化发言', '起身致意'],
      niceToHave: ['数据支撑', '守时'],
      forbidden: ['打断他人', '情绪化', '说空话'],
      tips: '政务场合核心是"守时、守规、守礼"：称呼准确（X局长/X处长），发言结构化（第一/第二/第三），不打断他人，不说大话空话。'
    },
    '销售服务礼仪（门店/汽车/珠宝）': {
      mustHave: ['微笑迎宾', '保持服务距离', '询问需求'],
      niceToHave: ['产品介绍结构', '异议处理'],
      forbidden: ['评判顾客购买力', '紧逼施压', '说竞品坏话'],
      tips: '销售礼仪核心是"尊重客户选择"：3-5米服务距离 + 开放式询问需求 + 先共情再解释价值 + 不评判不施压。'
    },
    '形象与职场仪表': {
      mustHave: ['TPO 原则', '着装得体', '仪容整洁'],
      niceToHave: ['5分钟快速调整', '场合意识'],
      forbidden: ['太随意', '太暴露', '过度装饰'],
      tips: '形象礼仪黄金法则：TPO原则（Time时间/Place地点/Object目的）。你的着装应该让同事和客户感到得体、被尊重。5分钟快速调整>0分。'
    }
  };
